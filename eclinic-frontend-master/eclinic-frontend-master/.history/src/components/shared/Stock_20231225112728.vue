  ...
  <v-expansion-panels mb-6 focusable>
    <v-expansion-panel>
      <v-card rounded="lg" class="ma-3">
        ...
        <v-card-text>
          <v-data-table v-if="items && items.inventory" :items="items.inventory" class="elevation-1" :items="stock"
            :headers="headers" sort-by="barcode" group-by="medicineName" :items-per-page="15">
