          ...
          <v-data-table class="elevation-1" :items="stock" :headers="headers" sort-by="barcode" group-by="medicineName"
            ...
            <template v-slot:group.header="{ items, isOpen, toggle }">
              <th colspan="5">
                ...
                {{ items[0].name }} -- Total Pieces: {{ items.length }}
...
export default {
  ...
  data: () => ({
    items: [{ name: "Barcode", length: "20", },
    { name: "Medicine Name", length: "25" },],
    stock: [],
    headers: [
            ...
            <v-expansion-panels mb-6 focusable>
              <v-expansion-panel>
                <v-card rounded="lg" class="ma-3">
                  ...
                  <v-card-text>
                    <v-data-table class="elevation-1" :items="stock" :headers="headers" sort-by="barcode"
                      group-by="medicineName" :items-per-page="15">
                      <template v-slot:group.header="{ items, isOpen, toggle }">
                        <th colspan="5">
                          <v-icon @click="toggle">
                            {{ isOpen ? "mdi-minus" : "mdi-plus" }}
