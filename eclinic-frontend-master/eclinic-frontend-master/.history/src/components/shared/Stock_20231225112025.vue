  ...
  <v-expansion-panels mb-6 focusable>
    <v-expansion-panel>
      <v-card rounded="lg" class="ma-3">
        ...
        <v-card-text>
          <v-data-table v-if="stock && stock.inventory" class="elevation-1" :items="stock.inventory" :headers="headers"
            sort-by="barcode" group-by="medicineName" :items-per-page="15">
