      ...
      <v-col cols="12">
        <v-card>
          <v-toolbar flat>
            ...
            <v-spacer></v-spacer>
            <router-link v-if="item && item.shortCode" :to="{ name: 'Add Stock', params: { id: item.shortCode } }">
...
export default {
  ...
  data: () => ({
    tab: 0,
    items: [],
    item: {}, // Initialize item as an empty object
  }),
  ...
  methods: {
    ...
    async fetchData() {
      ...
      // Set the value of this.item based on your requirements
      // For example, you can set it to the first item in the list
