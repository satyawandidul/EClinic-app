<template>
  <v-container fluid tag="section" id="importStock">
    <v-card rounded="lg">
      <v-card-title class="display-2">Import Stock
        <v-spacer></v-spacer>
        <v-file-input color="primary" label="Click here to import .csv file" accept=".csv" v-model="file">
        </v-file-input>
        <v-btn color="primary" small @click="loadData">Import</v-btn>
      </v-card-title>
      <v-card-text>
        <v-data-table class="elevation-1" :items="items" :headers="headers" sort-by="Batch" group-by="Medicine"
          :items-per-page="15">
